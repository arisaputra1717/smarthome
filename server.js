...
function createTableIfNotExists() {
    const createTableSQL = `
        CREATE TABLE IF NOT EXISTS konsumsi_energi (
            ...
            -- relay_status dan mqtt_topic akan ditambahkan oleh addRelayColumns
        )
    `;
...
// Fungsi helper untuk format tanggal (sudah ada)
...
// Fungsi helper baru untuk format jam (HH:MM)
function formatTimeForQuery(timeString) {
...
app.get('/api/statistics', (req, res) => {
    ...
    const startDate = req.query.start_date;
    const endDate = req.query.end_date;
    const startHour = req.query.start_hour; // New: Parameter jam mulai (HH:MM)
    const endHour = req.query.end_hour;     // New: Parameter jam akhir (HH:MM)
    ...
    let sql = `SELECT 
                ...
                MIN(kwh) as min_kwh,
                MAX(kwh) as max_kwh,
                MIN(tanggal || ' ' || waktu) as first_record_datetime,
                MAX(tanggal || ' ' || waktu) as last_record_datetime
              FROM konsumsi_energi 
              WHERE mac_address = ?`;
    ...
    db.get(sql, params, (err, row) => {
        ...
        res.json({
            mac_address: mac,
            period: (startDate && endDate ? `${startDate} ${startHour || '00:00'} to ${endDate} ${endHour || '23:59'}` : 'All time'),
            statistics: {
